...
export async function PUT(request: any){
    ...
    try{
        ...
        const follow = await UserProfile.findByIdAndUpdate(user, {$push: {following: followUser}}, {new: true}).exec()
        const follower = await UserProfile.findByIdAndUpdate(followUser, {$push :  {followers: user}}, {new: true})
        .populate('following')
        .populate('followers')
        .populate('blogs')
        .exec()
