...
const Slug = ({params}: {params: {slug: String}}) => {
    ...
    const debouncedHandleClap = useCallback(
        debounce((count: number) => {
            const currentCount = count + 1;
            console.log(`API call with clap count: ${currentCount}`);

            (async () => {
                const response = await fetch(`/api/blog/upvote/?id=${params.slug}`, {
                    method: 'PUT',
                    body: JSON.stringify({upvote: currentCount})
                })
                console.log(response.status)   
            })()
    ...
    const handleClap = () => {
        setClapCount(previous => previous + 1);
            ...
            <div className='col-span-1 flex flex-col  justify-center'>
                ...
                <div className="flex mt-10 items-center justify-between gap-4">
                    <div className="flex gap-5">
                        <h2 onClick={handleClap} className="text-primary no-select text-sm items-end flex">
