...
const BioModal = () => {
    ...
    const {user, bio, setBio, setShowBioModal, showBioModal, setUser} = React.useContext(UserContext);
  ...
  return (
    <div onClick={(e) => { e.preventDefault(); setShowBioModal(false); }} className={` ${showBioModal ? 'flex' : 'hidden'} fixed  z-30 w-full h-screen items-center justify-center top-0 left-0 bg-black bg-opacity-50`}>
         <div onClick={e => e.stopPropagation()} className="w-[600px] rounded-md bg-white py-8 px-6 flex flex-col gap-4">
            ...
            <div className="flex self-end gap-4 mt-8">
                ...
                <button onClick={() => setShowBioModal(false)} className="bg-white  text-green-600 border border-green-700 font-medium w-fit px-6 hover:bg-opacity-80 text-sm py-2 rounded-[15px]">Cancel</button>
            </div>
         </div>
    </div>
  )
