...
import SearchModal from "../Modals/SearchModal/page";
...
const ProductNavbar = ({ text = "stories" }: { text: string }) => {
  ...
  const [search, setSearch] = React.useState<string>("" as string);
  const [indexedData, setIndexedData] = React.useState<any>([]);
  const [searchResults, setSearchResults] = React.useState<any>([]);
  const [searchResultsPeople, setSearchResultsPeople] = React.useState<any>([]);
  const [searchResultPublication, setSearchResultPublication] =
    React.useState<any>([]);
  const [open, setOpen] = React.useState(false);

  const { setUser, blogs, setBlogs, bookmarks, setEmail } =
  ...
  let miniSearch = new MiniSearch({
    ...
    extractField: (document, fieldName) => {
      ...
      } else if (fieldName === "author.lastName") {
        return document.author.lastName;
      } else {
  ...
  React.useEffect(() => {
    if (indexedData.length < 1) return;
    ...
    setSearchResultsPeople(peopleResults);
    setSearchResultPublication(publicationResults);
  ...
  return (
    <div
      className={`md:px-10 px-7 py-3 border-b w-full bg-white z-10 flex justify-between items-center`}
    >
      <div className="flex gap-5 items-center">
        ...
        <div className="flex border relative border-slate-300 w-[300px] py-1.5 px-3 rounded-[20px]">
          <svg
          ...
          <input
            ...
            name=""
            id=""
            className="w-full ml-3 text-sm bg-transparent placeholder:text-sm placeholder:font-normal placeholder:text-slate-800 text-black outline-none"
          ...
          <SearchModal
            ...
            searchResultPublication={searchResultPublication}
          />
        </div>
      </div>
      ...
      <ul className="">
        <Link href="/">
          {" "}
          <li className="inline-block text-slate-800 font-normal text-sm px-3 py-2 rounded-md hover:bg-slate-100 cursor-pointer">
            Products
          </li>
        </Link>
        <Link href="/community">
      ...
      <div className="flex items-center gap-5 md:gap-8">
        {isProfile ? (
          <div
            onClick={() => push("/new-story")}
            className="flex gap-2 cursor-pointer hover:underline text-sm items-center"
          >
            <svg
              width="24"
              height="24"
        ...
        {showList && (
          <div className="fixed top-[50px] right-4 py-5 px-4 rounded-lg bg-white w-[15rem] min-h-[5rem] shadow-lg ">
            ...
            >
              <Link
                href="/profile"
                className="flex hover:bg-slate-100 cursor-pointer py-2 px-2 rounded-md items-center gap-2 text-black font-normal text-sm"
              >
                <svg
                  ...
                  viewBox="0 0 24 24"
                >
                  <path d="M3 7C3 6.44772 3.44772 6 4 6H20C20.5523 6 21 6.44772 21 7V8C21 8.55228 20.5523 9 20 9H4C3.44772 9 3 8.55228 3 8V7Z"></path>
                  <path d="M4 12C4 11.4477 4.44772 11 5 11H10C10.5523 11 11 11.4477 11 12V19C11 19.5523 10.5523 20 10 20H5C4.44772 20 4 19.5523 4 19V12Z"></path>
                  <path d="M14 11C13.4477 11 13 11.4477 13 12V19C13 19.5523 13.4477 20 14 20H19C19.5523 20 20 19.5523 20 19V12C20 11.4477 19.5523 11 19 11H14Z"></path>
                  <path d="M14.4453 2.16806C14.9048 1.8617 15.5257 1.98588 15.832 2.44541C16.1384 2.90493 16.0142 3.5258 15.5547 3.83216L12 6.20196L8.4453 3.83216C7.98577 3.5258 7.8616 2.90493 8.16795 2.44541C8.4743 1.98588 9.09517 1.8617 9.5547 2.16806L12 3.79826L14.4453 2.16806Z"></path>
                </svg>
                Profile
              </Link>
              <Link
                href="/new-story"
                className="flex hover:bg-slate-100 cursor-pointer px-2 py-2 rounded-md items-center gap-2 text-black font-normal text-sm"
              >
                <svg
                  data-testid="TradeIcon"
                  color="inherit"
                  width="14"
                  height="14"
                  viewBox="0 0 24 24"
                >
                  <path
                    d="M7 6C7 6.55228 7.44772 7 8 7H18C18.5523 7 19 6.55228 19 6C19 5.44772 18.5523 5 18 5H8C7.44772 5 7 5.44772 7 6Z"
                    fillRule="evenodd"
                    clipRule="evenodd"
                  ></path>
                  <path
                    d="M17 18C17 18.5523 16.5523 19 16 19H6C5.44772 19 5 18.5523 5 18C5 17.4477 5.44772 17 6 17H16C16.5523 17 17 17.4477 17 18Z"
                    fillRule="evenodd"
                    clipRule="evenodd"
                  ></path>
                  <path
                    d="M9.70711 9.70711C10.0976 9.31658 10.0976 8.68342 9.70711 8.29289L7.41421 6L9.70711 3.70711C10.0976 3.31658 10.0976 2.68342 9.70711 2.29289C9.31658 1.90237 8.68342 1.90237 8.29289 2.29289L5.29289 5.29289C4.90237 5.68342 4.90237 6.31658 5.29289 6.70711L8.29289 9.70711C8.68342 10.0976 9.31658 10.0976 9.70711 9.70711Z"
                    fillRule="evenodd"
                    clipRule="evenodd"
                  ></path>
                  <path
                    d="M14.2929 21.7071C13.9024 21.3166 13.9024 20.6834 14.2929 20.2929L16.5858 18L14.2929 15.7071C13.9024 15.3166 13.9024 14.6834 14.2929 14.2929C14.6834 13.9024 15.3166 13.9024 15.7071 14.2929L18.7071 17.2929C19.0976 17.6834 19.0976 18.3166 18.7071 18.7071L15.7071 21.7071C15.3166 22.0976 14.6834 22.0976 14.2929 21.7071Z"
                    fillRule="evenodd"
                    clipRule="evenodd"
                  ></path>
                </svg>
              ...
              >
                <svg
                  ...
                  viewBox="0 0 24 24"
                >
                  <path d="M16.375 12C16.375 12.9665 15.6085 13.75 14.663 13.75H5.83696C4.89147 13.75 4.125 12.9665 4.125 12C4.125 11.0335 4.89147 10.25 5.83696 10.25H14.663C15.6085 10.25 16.375 11.0335 16.375 12Z"></path>
                  <path d="M11.6376 18.4874C10.9541 17.804 10.9541 16.696 11.6376 16.0126L15.6501 12L11.6376 7.98744C10.9541 7.30402 10.9541 6.19598 11.6376 5.51256C12.321 4.82915 13.429 4.82915 14.1124 5.51256L19.3624 10.7626C20.0459 11.446 20.0459 12.554 19.3624 13.2374L14.1124 18.4874C13.429 19.1709 12.321 19.1709 11.6376 18.4874Z"></path>
                </svg>
                Log out
              </button>
            </ul>
              ...
              {bookmarks.length > 0 ? (
                bookmarks.map((bookmark: any, index: any) => (
                  <div
                    ...
                    key={index}
                    className="flex cursor-pointer border-l-2 pl-5 border-green-600 gap-5 items-center"
                  >
                ...
                <p className="text-gray-600 mt-10 font-semibold text-sm">
                  No new bookmarks.
                </p>
              )}
            </div>
